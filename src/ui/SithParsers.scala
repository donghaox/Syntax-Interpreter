...
class SithParsers extends RegexParsers {
   ...
   def term : Parser[Expression]= obj|iteration|assignment|deref|lambda | block | literal | identifier | "("~> expression <~ ")"
  ...
  //Sith Parsers
   ...
   {
     case i ~"=" ~  exp => Assignment(i,exp);
   }
   
   def iteration:Parser[Expression] = "while" ~> "(" ~> (expression <~ ")") ~ expression ^^
   {
     case exp ~ condition => Iteration(exp,condition)
   }
   
   def deref = "[" ~> expression <~ "]" ^^
   {
     case exp => FunCall(Identifier("val"), List(exp))
   }
   //implementing objects
   def obj: Parser[Expression] = "object"~ "{" ~ expression ~ rep(";"~>expression) ~ "}"~opt("extends" ~> identifier) ^^
   {
     case "object" ~ "{" ~ exp ~ Nil ~ "}" ~ None => Object{List(exp)}
     case "object" ~ "{" ~ exp ~ list ~ "}" ~ None => Object(exp::list)
     case "object" ~ "{" ~ exp ~ list ~ "}" ~ Some(id) => Object(exp::list, id)
   }

def access: Parser[Expression] = term~opt("."~>identifier) ^^{ 
  case term ~ None => term
  case term ~ Some(id) => Access(term,id)
}
}